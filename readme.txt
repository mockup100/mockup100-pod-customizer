...
By activating the plugin and entering a tenant API key, the site administrator agrees to the linked terms on behalf of the site.
...
Some advanced features in the in-page designer toolbar call **optional paid Mockup100 SaaS endpoints**. Free users can keep using the rest of the designer; the paid endpoints simply return a "subscription required" error from the SaaS side when invoked without an active subscription, and the free plugin code itself does NOT contain any license-key check or paywall gate.

* **Grading workspace** (designer toolbar "Grading" button) — opens a multi-size pattern-grading workspace that renders sized variants on the Mockup100 SaaS render farm. Requires an active Mockup100 Grading subscription ($9 Standard / $19 Enterprise per month, billed by Mockup100 — not by this plugin). New Mockup100 accounts can start a **free 30-day Grading trial** (no credit card required); the trial is opt-in, can be cancelled at any time, and is managed entirely on the Mockup100 SaaS side. Subscription and trial details open at `https://www.mockup100.com/pricing#grading`. Endpoint: `https://www.mockup100.com/api/v1/runtime/templates/{id}/grading/compose`.
* **Order edit page Grading entry** (wp-admin order edit page side meta box "Mockup100 Grading (optional add-on)") — adds a "Generate Grading Sizes" button next to each Mockup100-bound order line item. Visible only to users with the `edit_shop_orders` capability. The first click shows a browser confirmation dialog disclosing the destination domain (`https://www.mockup100.com`) and the fields that will be sent (template id, part keys); no request leaves the site until the user explicitly accepts. The same Mockup100 Grading subscription described above is required, and the plugin honours `Mockup100 > Settings > Order Grading Entry` so site administrators can disable the meta box and its REST route at any time. Endpoint: `https://www.mockup100.com/api/v1/external/runtime/templates/{id}/grading/compose`.
* **Cloud token top-up** (Mockup100 Settings page "Buy Tokens" link) — opens `https://www.mockup100.com/pricing#tokens` for optional pay-as-you-go credits used for HD render output and premium artwork licensing. The 512 px preview render is free for everyone.
* **WordPress Pro Add-on** (Mockup100 Settings page "Get WP Pro" link) — opens `https://www.mockup100.com/pricing#wp-pro` for a separate, separately-distributed paid WordPress plugin (`mockup100-pod-customizer-pro`) for bulk order export and team permissions. It is NOT bundled in this free plugin, is never required for the free features to work, and no part of that paid plugin is shipped, downloaded, auto-activated, or invoked by this free plugin.
...
* Add a wp-admin order edit page entry point for the optional Mockup100 Grading SaaS feature, designed against the WordPress.org plugin guidelines (no phoning home without disclosure, no admin hijack, no in-plugin paywall, full External Services disclosure, and a site-level kill switch).
    ...
    * New REST route `POST /wp-json/mockup100/v1/order/{id}/grading/compose` proxies the request through `Mockup100_Api_Proxy::compose_grading()` (server-to-server, tenant API key never reaches the browser). Subscription / tier checks remain on the Mockup100 SaaS side: an unsubscribed tenant simply receives a `subscription_required` response which the wp-admin UI translates into a "Open subscription page" button pointing at `https://www.mockup100.com/pricing#grading`. The plugin code itself contains no license-key check.
...
= 0.5.4 =
* Merge WDplugin `4d17c15` disclosure update into the current 0.5.4 package: the optional Grading workspace documentation now explicitly states that new Mockup100 accounts can start a free 30-day Grading trial with no credit card required, managed entirely on the SaaS side. The plugin remains free and contains no license-key check, paywall gate, or trial-state tracking.
